...
class NSGA2():
    ...
    def initial_population_with_equal_scale(self):
        ...
        for m in range(self.pop_size):
            create_indexes = np.random.choice(a=[i for i in range(self.x_dim)], size=self.new_provider_count,
                                              replace=False)
    ...
    def fitness(self, demands_provider_np, demands_pdd_np, popu):
        ...
        for i in range(actural_popsize):
            ...
            # 居民区 可达性适应度值，该值越大越好
            y1_values_double[i] = self.calculate_global_ave_accessibility_numpy(demands_pdd_np)
        ...
        # 统一转成最小化问题
        pop_fitness = np.vstack((10 / y1_values_double, 1000 * y2_values_double, 20 / y3_values_double)).T
        return pop_fitness

    def calculate_global_ave_accessibility_numpy(self, demands_pdd_np):
        """
        计算全局可达性的总和
        :param demands_numpy:
        :return:
        ...
        print(np.nanmedian(demands_pdd_np[:, 0] * demands_pdd_np[:, 1]))
        return np.nanmedian(demands_pdd_np[:, 0] * demands_pdd_np[:, 1])
    ...
    def non_donminate(self,objectives_fitness):  # pop*2行
        fronts = []  # Pareto前沿面
        fronts.append([])
        set_sp = []
    ...
    def draw_fitness_graph(self, demands_provider_np, demands_pdd_np, result_population):  # 画图
        ...
        if self.f_num==2:
            ...
            # plt.plot(,'--',label='')
            plt.axis(
                [np.min(objectives_fitness[:, 0]), np.max(objectives_fitness[:, 0]), np.min(objectives_fitness[:, 1]),
                 np.max(objectives_fitness[:, 1])])  # test ????????
    ...
    def excute(self, demands_provider_np, demands_pdd_np):  # 主程序
        ...
        self.draw_fitness_graph(demands_provider_np, demands_pdd_np, result_population)
        # print(self.fronts)
...
if __name__ == '__main__':
    ##########################基本情况说明####################################
    #建设方式：建设数量确定，规模确定的provider，符合3个目标函数
    # 数量是建2个，每个的规模都是10
    ##########################基本情况说明####################################
    ...
    # 获取到所有的记录
    demands_provider_np, demands_pdd_np, provider_id_list = mongo_operater_obj.find_records_format_np_theory(0,
                                                                                                             DEMANDS_COUNT,
