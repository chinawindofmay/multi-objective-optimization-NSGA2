...
class NSGA3:
    ...
    def fitness(self, demands_provider_np, demands_pdd_np, popu):
        ...
        pop_fitness = np.vstack((1/y1_values_double, 1000*y2_values_double, 20/y5_values_double, 0.0001*y6_values_double)).T
        return pop_fitness
    ...
    # 方式1：定规模、定个数
    def initial_population_with_scale_count(self, popsize):
        pop = np.full(shape=(popsize, self.petential_new_providers_count), fill_value=0)
        for m in range(popsize):
            index_1 = np.random.randint(low=0, high=self.petential_new_providers_count)
            index_2 = np.random.randint(low=0, high=self.petential_new_providers_count)
            while(index_1==index_2):
                index_2 = np.random.randint(low=0, high=self.petential_new_providers_count)
            creating_mark = np.full(shape=(self.petential_new_providers_count),fill_value=0)
            creating_mark[index_1]=1
            creating_mark[index_2]=1       #数量是建2个
            pop[m, :] = creating_mark*10    #规模都是10
            if np.argwhere(pop[m, :]  == 10).flatten().shape[0] < 2:
                print("test")
        return pop
    # # 方式2：不定规模、定个数
    # def initial_population_without_constraint(self, popsize):
    #
    # # 方式3：不定规模、定总数
    # def initial_population_without_constraint(self, popsize):
    ...
    def excute(self, demands_provider_np, demands_pdd_np):
        ...
        # 随机初始化种群
        popu = self.initial_population_with_scale_count(actural_popsize)   #定规模、定数量
        print(np.sum(popu > 0, 0))
        test_popu=test_popu_check(popu)
        # popu = self.initial_population_without_constraint(actural_popsize)
        ...
        for i in range(self.GENERATIONS):
            ...
            off_spring_popu = self.crossover_and_mutation_simple_with_scale_count(popu[matingpool,:],  self.pc, self.pm)# 遗传算子,模拟二进制交叉和多项式变异
            # print(np.sum(off_spring_popu > 0, 0))
            test_popu = test_popu_check(off_spring_popu)
            off_spring_fitness = self.fitness(demands_provider_np, demands_pdd_np, off_spring_popu)  # 计算适应度函数
...
def test_popu_check(result_popu):
    popu_index = []
    for i in range(result_popu.shape[0]):
        popu_index.append(np.argwhere(result_popu[i, :]  == 10).flatten().shape[0])
    return np.array(popu_index)

def test_get_count(file_content):
    '''
    计算每组解出现的次数
...
def test_show_bar(uniques):
    # 设置matplotlib正常显示中文和负号
    matplotlib.rcParams['font.sans-serif'] = ['SimHei']  # 用黑体显示中文
    matplotlib.rcParams['axes.unicode_minus'] = False  # 正常显示负号
    x_label=[]
    x_label=['['+str(i[0])+ ',' +str(i[1])+']' for i in uniques.tolist()]
    plt.bar(x_label, uniques[:, 2])
    plt.xticks(size=14,rotation=60)
    plt.yticks(size=14)
    plt.show()


def test_frequency_show(result_provider_id_popu):
    result = test_get_count(result_provider_id_popu)
    test_show_bar(result)
...
if __name__=="__main__":
    ...
    test_frequency_show(result_provider_id_popu)
    # # 输出
    # for i in range(result_popu.shape[0]):
    #     print(result_popu[i, :])