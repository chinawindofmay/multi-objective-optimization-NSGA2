...
def tournament_selection2(chromo,N):
    ...
    for i in range(a):
        ...
        if (chromo_rank[0]==chromo_rank[1]):
            ...
            chromo_parent.append(chromo[int(chromo_candidate[maxchromo_candidate])])##############修改
        else:
            chromo_parent.append(chromo[int(chromo_candidate[minchromo_candidate])])###############修改
...
def cross_mutation(chromo_parent,f_num,x_num,x_min,x_max,pc,pm,yita1,yita2,fun):
    ...
    chromo_offspring=[]
    #随机选取两个父代个体
    for i in range(round(len(chromo_parent))):##################################修改
        ...
        if(random.random()<pc):
            ...
            for j in range(x_num):
                ...
                off11=float(0.5*((1+gama)*parent1.x[j]+(1-gama)*parent2.x[j]))##############################修改
...
def elitism(N,combine_chromo2,f_num,x_num):
    ...
    while (aaa<index2):
        ...
        aaa=aaa+1
    temp_crowd=sorted(temp,key=lambda Individual:Individual.nnd,reverse=True)######################修改
    remainN=N-index1
