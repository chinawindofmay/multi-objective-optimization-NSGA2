...
class RPQueue(object):
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        self._batch_request_of_rpqueue(self.request_cache)
    ...
    def __iadd__(self, route_planner_object):
        ...
        if route_planner_object.batchable:
            ...
            if len(self.request_cache) == type(self).BATCH_SIZE:
                self._batch_request_of_rpqueue([r for r in self.request_cache])
                self.request_cache.clear()
        else:
            self._standalone_request_of_rpqueue(route_planner_object)
        return self
    ...
    函数：发送非批量请求
    """
    def _standalone_request_of_rpqueue(self, route_planner_object):
        cls = type(self)
        ...
        try:
            ...
            if 200 <= reply.status_code < 300:
                #此处直接调用了类的__call__方法
                self.on_each_ok_callback_save_data(reply.json(), route_planner_object)
        except:
            # traceback.print_exc()
    ...
    函数：发送批量请求
    """
    def _batch_request_of_rpqueue(self, route_planner_objects):
        ...
        try:
                    ...
                    if 'status' in route_planner_response:
                        if 200 <= route_planner_response['status'] < 300 and self.on_each_ok_callback_save_data is not None:
                            try:
                                ...
                                self.on_each_ok_callback_save_data(route_planner_response['body'], route_planner_object)
                            except:
                                traceback.print_exc()
        except:
            # traceback.print_exc()
            ...
            for route_planner_object in route_planner_objects:
                self.on_each_ok_callback_save_data(route_planner_response_body=None,route_planner_object=route_planner_object,batch_fail=True)
    """
    函数：吞吐量控制
    """
