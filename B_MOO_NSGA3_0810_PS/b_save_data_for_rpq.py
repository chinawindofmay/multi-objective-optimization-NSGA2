...
class SaverDataOfRPQ(object):
    ...
    """
    def __call__(self, route_planner_response_body, route_planner_object,batch_fail=False):
        result={}
        #第一步：判断batch是否成功了？
        if batch_fail:
            result={"time":100}
        ...
        if len(poi_attr_dict) == 1:###一种Drivinger类型
            #更新到数据库
            self._update_query_one_provider_per_time(demand_id, provider, poi_attr_dict)
            # 删除掉已经全部入库的记录了
    ...
    函数：每一个provider，提交一次；
    """
    def _update_query_one_provider_per_time(self, demand_id,provider, poi_attr):
        #获得预先已经存在provider及其travel
        travel=provider["travel"]
        travel["D_T"]=poi_attr["Drivinger"]["time"]
        #更新一条记录
