...
def DTLZ1_f1(population):
    sum1 = np.array(
        [np.sum(np.power(population[i, 2:] - 0.5, 2) - np.cos(20 * np.pi * (population[i, 2:] - 0.5))) for i in
         range(len(population[:, 0]))])
    x_num=len(population[0,:])
    g = (100 * (x_num - 2) + 100 * sum1).astype(np.float)
    f1 = ((1 + g) * population[:, 0] * population[:, 1]).astype(np.float)
    return f1


def DTLZ1_f2(population):
    sum1 = np.array(
        [np.sum(np.power(population[i, 2:] - 0.5, 2) - np.cos(20 * np.pi * (population[i, 2:] - 0.5))) for i in
         range(len(population[:, 0]))])
    x_num=len(population[0,:])
    g = (100 * (x_num - 2) + 100 * sum1).astype(np.float)
    f2 = ((1 + g) * population[:, 0] * (1 - population[:, 1])).astype(np.float)
    return f2

def DTLZ1_f3(population):
    sum1 = np.array(
        [np.sum(np.power(population[i, 2:] - 0.5, 2) - np.cos(20 * np.pi * (population[i, 2:] - 0.5))) for i in
         range(len(population[:, 0]))])
    x_num=len(population[0,:])
    g = (100 * (x_num - 2) + 100 * sum1).astype(np.float)
    f3 = ((1 + g) * (1 - population[:, 0])).astype(np.float)
    return f3
