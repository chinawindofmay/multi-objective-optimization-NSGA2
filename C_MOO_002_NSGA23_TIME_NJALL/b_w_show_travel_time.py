...
if __name__ == '__main__':
    ...
    # 获取到所有的记录
    demands_provider_np, provider_id_list,demand_id_list = mongo_operater_obj.find_records_travel_time_NJALL(0, DEMANDS_COUNT, PROVIDERS_COUNT,SMALL_INF)  # 必须要先创建索引，才可以执行

    ODs = pd.DataFrame(data=demands_provider_np,columns=["dx","dy","px","py","t"])

    ODs.to_csv("./log/tzzs_data.csv")
