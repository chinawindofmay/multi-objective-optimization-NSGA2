...
class PS_STA_dynamic():
    ...
    def calculate_inequality_np(self, demands_pdd_np):
        ...
        """
        # 不同时刻的总人口
        pop_08_sum = np.nansum(demands_pdd_np[:, 0])  #152718
        pop_13_sum = np.nansum(demands_pdd_np[:, 1])  #129813
        pop_18_sum = np.nansum(demands_pdd_np[:, 2])  #155872
        pop_22_sum = np.nansum(demands_pdd_np[:, 3])  #103569
        SUM_CHARGE=self.E_OLD_SUM  #3925
        # 可达性差异==不公平性最小化 最小化为目标
        inequlity=np.full(shape=(demands_pdd_np.shape[0],4),fill_value=0.0)
        inequlity[:,0]= (demands_pdd_np[:, 4] - SUM_CHARGE / pop_08_sum)
        inequlity[:,1]= (demands_pdd_np[:, 5] - SUM_CHARGE / pop_13_sum)
        inequlity[:,2]= (demands_pdd_np[:, 6] - SUM_CHARGE / pop_18_sum)
        inequlity[:,3]= (demands_pdd_np[:, 7] - SUM_CHARGE / pop_22_sum)
        return inequlity
    ...
    def update_accessibility_to_shp_table(self,demand_id_list, demands_pdd_np_result,shp_table_name,conn,cursor):
        ...
        for row in range(Besty.shape[0]):
            update_sql = "update {5} set access08={0},access13={1},access18={2},access22={3} where KEYID_POPU='{4}'".format(
    ...
    def update_inequlity_to_shp_table(self,demand_id_list, inequlity,shp_table_name,conn,cursor):
        for row in range(inequlity.shape[0]):
            update_sql = "update {5} set equity08={0},equity13={1},equity18={2},equity22={3} where KEYID_POPU='{4}'".format(
...
if __name__ == '__main__':
    ...
    SMALL_INF=1e-6
    SHP_TABLE_NAME = "popu0412_join_parks_join_ps"  ### 面图层名称，而非点图层
    ...
    # 获取到所有的记录
    demands_provider_np, demands_pdd_np, provider_id_list,demand_id_list = mongo_operater_obj.find_records_access_PS_NJALL_TIME(0, DEMANDS_COUNT, OLD_PROVIDERS_COUNT + PENTENTIAL_NP_C,
