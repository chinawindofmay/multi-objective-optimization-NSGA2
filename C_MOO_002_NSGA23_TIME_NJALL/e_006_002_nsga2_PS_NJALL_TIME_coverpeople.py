...
class NSGA2():
    ...
    # @clock
    def fitness_nsga2_njall(self, demands_provider_np, demands_pdd_np, popu):
        """
        目标函数1：行驶时间总和最小；
        ...
        y1_cover_people = np.full(shape=(actural_popsize,), fill_value=1, dtype=np.float32)  # 行驶时间
        ...
        for i in range(actural_popsize):
            ...
            effective_providers_np = self.create_providers_np(demands_provider_np_temp, demands_pdd_np_temp, solution_join)
            # demands_provider_np 计算vj
            self.update_provider_vj(demands_provider_np_temp,effective_providers_np)
            ...
            y1_cover_people[i] = self.calcuate_global_cover_people_2(effective_providers_np)
            # y1_cover_people[i] = self.calcuate_global_cover_people(demands_provider_np_temp,solution_join)
            # 建设成本，越小越好
        ...
        # 在对机器学习的数据进行预处理时，往往都需要归一化，从而避免因特征值范围不同而造成学习结果偏差。
        #归一化
        y1_cover_people=(y1_cover_people-np.min(y1_cover_people))/(np.max(y1_cover_people)-np.min(y1_cover_people))
        y3_inequlity=(y3_inequlity-np.min(y3_inequlity))/(np.max(y3_inequlity)-np.min(y3_inequlity))
        y5_cost=(y5_cost-np.min(y5_cost))/(np.max(y5_cost)-np.min(y5_cost))
        pop_fitness = np.vstack((y1_cover_people, y3_inequlity,y5_cost)).T
        return pop_fitness
    ...
    def driving_time_waiting_time(self, demands_provider_np, demands_pdd_np,solution_join):
        ...
        for k in range(0, 4):
            ...
            mask= solution_join >= self.SMALL_INF  # 已建的会直接赋值solution_join为SMALL_INF，所以需要等于  #总需求量1294,新建的直接赋值规模值
            # 充电桩供给量 M_J
            M_J=demands_provider_np[:,0,0][mask]+solution_join[mask]   #原有总的供给量 4043，新建设施10000
    ...
    def driving_time_one_moment(self,W_I,  TRAVELTIME_IJ, Y_IJ):
        ...
        time_mask=TRAVELTIME_IJ<10   ###需要考虑到超限的情况
        return np.nansum(np.dot(t1, np.where(time_mask,TRAVELTIME_IJ,self.SMALL_INF).T))
    ...
    def calcuate_global_cover_people_2(self, effective_providers_np):
        """
        获取覆盖全局人口的总和
        :param demands_np:
        :return:
        ...
        vj=0
        for k in range(0, 4):
            vj_np = np.full((effective_providers_np.shape[0], 1), 0.000000001)
            for j in range(effective_providers_np.shape[0]):
                mask = effective_providers_np[j, :, 4 + k] <= self.THROD  # 对交通时间做对比
                vj_np[j, 0] = np.sum(
                        np.where(mask, effective_providers_np[j, :, 0 + k], self.SMALL_INF)  # 人口
                )
            vj=vj+np.nansum(vj_np)
        # 将最大化问题转为最小化问题
        return 100000000/vj
    ...
    def create_providers_np(self, demands_provider_np, demands_pdd_np, solution_join):
        ...
        # 将solution部分增加进去
        mask = solution_join == 0  #说明既不是未建，也不是新建
    ...
    # @clock
    def update_provider_vj(self, demands_provider_np, effective_providers_np):
        """
        计算vj
        :param demands_np:
        :param DEMANDS_COUNT:
        ...
        for k in range(0, 4):
            ...
            for j in range(effective_providers_np.shape[0]):
                ...
                vj_np[j,0]=np.sum(
                    np.divide(
                        np.where(mask,effective_providers_np[j, :, 0+k],self.SMALL_INF)   #人口
                        , np.power(np.where(mask,effective_providers_np[j, :, 4+k],self.BIG_INF), self.BEITA)   #时间
                    )
                )
            # 更新vj列
        ...
        # for k in range(0, 4):
        #     vj_np = np.full((effective_providers_np.shape[0], 1), 0.000000001)
        #     mask = effective_providers_np[:, :, 4 + k] <= self.THROD  # 对交通时间做对比
        #     vj_np[:,0] = np.sum(
        #         np.divide(
        #             np.where(mask,effective_providers_np[:, :, 0 + k], self.SMALL_INF)  # 人口
        #             , np.power(np.where(mask,effective_providers_np[:, :, 4 + k],self.BIG_INF), self.BEITA)  # 时间
        #         ),axis=1)
        #     # 更新vj列
        #     demands_provider_np[:, :, 5 + k] = np.tile(vj_np, (1, demands_provider_np.shape[1]))
        #     # test = np.tile(vj_np, (1, demands_provider_np.shape[1]))
    # </editor-fold>
    ##############################################end:fitness#########################################################
    ...
    def show_initial_population_fitness_graph(self, demands_provider_np, demands_pdd_np, initial_population):  # 画图
        ...
        """
        # 评价函数
        objectives_fitness = self.fitness_nsga2_njall(demands_provider_np, demands_pdd_np, initial_population)
        if self.f_num==2:
            x = []
            y = []
            for i in range(objectives_fitness.shape[0]):
            ...
            # plt.xlabel('建成年份', font2)
            # plt.ylabel('入住率水平（人/百平方米）', font2)
            ax.set_xlabel("覆盖人口反比", font2)
            ax.set_ylabel("不公平性", font2)
            ax.set_zlabel("建设成本", font2)
            # ax.set_title('初始化种群适应度分布图', font2)
            type1 = ax.scatter(objectives_fitness[:, 0], objectives_fitness[:, 1], objectives_fitness[:, 2], s=30,c='g')
    ...
    def show_pair_plot_of_fitness(self, objectives_fitness):
        ...
        df4.columns = ['覆盖人口反比', '不公平性', '建设成本', "initial"]
    ...
    def show_fitness_graph_and_pairgraph(self, demands_provider_np, demands_pdd_np, population_front0):  # 画图
        ...
        """
        # 评价函数
        objectives_fitness = self.fitness_nsga2_njall(demands_provider_np, demands_pdd_np, population_front0)
        if self.f_num==2:
            F1 = []  # equlity
            F2 = []  # cover people
            for i in range(objectives_fitness.shape[0]):
            ...
            self.show_pair_plot_of_fitness(objectives_fitness)
            # 将结果存储下来
            np.savez("./log/njall_solutions_nsga2_up200_time033_coverpeople", population_front0=population_front0,objectives_fitness=objectives_fitness)
    ...
    def excute_nsga2(self, demands_provider_np, demands_pdd_np, provider_id_list):
        ...
        for i in range(self.max_iteration):
            ...
            # 评价函数
            objectives_fitness = self.fitness_nsga2_njall(demands_provider_np, demands_pdd_np, population_child_conbine)
            # 快速非支配排序
            ...
            print(i,"代")
        objectives_fitness = self.fitness_nsga2_njall(demands_provider_np, demands_pdd_np, population)
        fronts = self.non_donminate(objectives_fitness)
        # fronts=self.fast_non_dominated_sort(objectives_fitness)
...
def main_849_873_CUF_SUF_njall():
    # 参数设置  代
    ...
    # 总规模
    E_seting = 11000    #总量是
    # --已建：已建充电站：633，已建充电桩3925
    # --目标：公共充电站860座
    # 充电桩规模达11448个，配建停车类用地布局充电站365座，可供布局充电桩3644个。
    # --1225，15092
    # --待建：590，11167
    low2 = 20  # 最低阈值，要求总量除以最低阈值不小于候选点
    up2 = 200  # 最高阈值，最多建设10个
    ...
    # 获取到所有的记录
    demands_provider_np, demands_pdd_np, provider_id_list = mongo_operater_obj.find_records_PS_NJALL_TIME(0, DEMANDS_COUNT, OLD_PROVIDERS_COUNT + PENTENTIAL_NP_C, SMALL_INF)  # 必须要先创建索引，才可以执行
