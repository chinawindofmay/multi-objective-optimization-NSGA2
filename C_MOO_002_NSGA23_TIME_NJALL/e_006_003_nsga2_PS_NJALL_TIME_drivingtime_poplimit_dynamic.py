...
class NSGA2():
    ...
    def show_fitness_graph_and_pairgraph(self, demands_provider_np, demands_pdd_np, population_front0):  # 画图
        ...
        if self.f_num==2:
            ...
            self.show_pair_plot_of_fitness(objectives_fitness)
            # 将结果存储下来
            np.savez("./log/njall_solutions_nsga2_up100_time0.16_poplimit_dynamic", population_front0=population_front0,objectives_fitness=objectives_fitness)
...
def main_849_873_CUF_SUF_njall():
    ...
    # 获取到所有的记录
    demands_provider_np, demands_pdd_np, provider_id_list = mongo_operater_obj.find_records_PS_NJALL_TIME_poplimit_dynamic(0, DEMANDS_COUNT, OLD_PROVIDERS_COUNT + PENTENTIAL_NP_C, SMALL_INF)  # 必须要先创建索引，才可以执行
