...
class NSGA2():
    ...
    def fitness_nsga2_njall(self, demands_provider_np, demands_pdd_np, popu):
        ...
        #归一化
        y1_driving_time=(y1_driving_time-np.min(y1_driving_time))/(np.max(y1_driving_time)-np.min(y1_driving_time))
        ...
        pop_fitness = np.vstack((y1_driving_time, y3_inequlity,y5_cost)).T
        return pop_fitness
    ...
    def driving_time_waiting_time(self, demands_provider_np, demands_pdd_np,solution_join):
        ...
        :return:
        """
        waiting_time_array=np.full(shape=(1,), fill_value=0.0, dtype=np.float32)
        driving_time_array=np.full(shape=(1,), fill_value=0.0, dtype=np.float32)
        for k in range(0, 1):
            # 需求量 W_I
    ...
    def test_global_accessibility_numpy(self, demands_pdd_np):
        ...
        print("理论值：",self.E_OLD_SUM+self.E_NEW_SUM,"计算得值：", np.nansum(demands_pdd_np[:, 0]*demands_pdd_np[:, 1]))
    ...
    def calculate_global_inequality_np(self, demands_pdd_np):
        ...
        # 可达性差异最小化为目标
        return (np.nansum((demands_pdd_np[:, 1] - SUM_CHARGE / pop_08_sum) ** 2) )
    ...
    def update_provider_vj(self, demands_provider_np, demands_pdd_np, solution_join):
        ...
        for j in range(effective_providers_np.shape[0]):
            ...
            vj_np[j,0]=np.sum(
                np.divide(
                    np.where(mask,effective_providers_np[j, :, 0],self.SMALL_INF)   #人口
                    , np.power(np.where(mask,effective_providers_np[j, :, 1],self.BIG_INF), self.BEITA)   #时间
                )
            )
        # 更新vj列
    ...
    def show_pair_plot_of_fitness(self, objectives_fitness):
        ...
        df4.columns = ['Travel time', 'Inequity', 'Cost', "initial"]
    ...
    def show_fitness_graph_and_pairgraph(self, demands_provider_np, demands_pdd_np, population_front0):  # 画图
        ...
        if self.f_num==2:
            ...
            self.show_pair_plot_of_fitness(objectives_fitness)
            # 将结果存储下来
            np.savez("./log/njall_solutions_nsga2_up100_time0.16_poplimit_static", population_front0=population_front0,objectives_fitness=objectives_fitness)
...
def main_849_873_CUF_SUF_njall():
    ...
    # 获取到所有的记录
    demands_provider_np, demands_pdd_np, provider_id_list = mongo_operater_obj.find_records_PS_NJALL_TIME_poplimit_static(0, DEMANDS_COUNT, OLD_PROVIDERS_COUNT + PENTENTIAL_NP_C, SMALL_INF)  # 必须要先创建索引，才可以执行
