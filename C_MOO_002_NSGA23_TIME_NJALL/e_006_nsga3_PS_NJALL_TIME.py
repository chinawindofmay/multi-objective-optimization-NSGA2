...
class NSGA3():
    ...
    def update_provider_vj(self, demands_provider_np, demands_pdd_np, solution_join):
        ...
        for k in range(0, 4):
            ...
            for j in range(effective_providers_np.shape[0]):
                ...
                test=np.divide(effective_providers_np[j, :, 0+k][mask], np.power(effective_providers_np[j, :, 4+k][mask], self.BEITA))
                mean_test=np.mean(test)
                min_test=np.min(test)
                max_test=np.max(test)
                max_place=np.argmax(test)
            # 更新vj列
    ...
    def excute_nsga3(self, demands_provider_np, demands_pdd_np, provider_id_list):
        ...
        # 绘制初始解的折线分布图
        self.show_initial_line_graph(popu_fitness)
        ...
        # 绘制初始解的折线分布图
        self.show_initial_line_graph(popu_fitness)
        np.savez("./log/njall_solutions_nsga3", population_front0=popu, objectives_fitness=np.array(popu_fitness[i, :]))

    def show_initial_line_graph(self, popu_fitness):
        fig1 = plt.figure()
        ...
        plt.xlabel('Objectives', font2)
        plt.ylabel('Fitness', font2)
        plt.xlim([1, self.M])
        for i in range(popu_fitness.shape[0]):
            plt.plot(range(1,1+self.M),np.array(popu_fitness[i, :]))
        plt.show()


def main_873_849_CUF_SUF_njall():
    # 参数设置  代
    ...
    # 定义NSGA和执行主程序
    nsga3_obj = NSGA3(pc2, pm2, low2, up2, OLD_PROVIDERS_COUNT, THROD, BEITA, PENTENTIAL_NP_C, POP_SIZE2,
                      N_GENERATIONS2, f_num,
                      is_builded_providers_adjust, B_NP_CF,
                      B_NP_C, B_NP_SF,
                      B_NP_S, E_seting,SMALL_INF,BIG_INF)
    nsga3_obj.excute_nsga3(demands_provider_np, demands_pdd_np, provider_id_list)


# NSGA3入口
if __name__ == '__main__':
    #数量不固定、规模不固定、总量固定
    main_873_849_CUF_SUF_njall()
